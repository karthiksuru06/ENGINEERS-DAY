...
-- This migration is completely FRESH-DATABASE and EXISTING-DATABASE safe.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS xpasses (
  id            UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  profile_id    UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
  xpass_id      TEXT UNIQUE NOT NULL,
...
CREATE TABLE IF NOT EXISTS events (
  ...
  registration_xp   INTEGER NOT NULL DEFAULT 10,
  checkin_xp        INTEGER NOT NULL DEFAULT 20,
...
-- ============================================================
-- EXISTING DATA BACKFILL & SCHEMA PATCHES
-- ============================================================

DO $$ 
BEGIN
  -- Add columns if they missed the CREATE TABLE (because table existed)
  ...
  ALTER TABLE events ADD COLUMN IF NOT EXISTS academic_year_id UUID REFERENCES academic_years(id) ON DELETE SET NULL;
  ALTER TABLE events ADD COLUMN IF NOT EXISTS registration_xp INTEGER NOT NULL DEFAULT 10;
  ALTER TABLE events ADD COLUMN IF NOT EXISTS checkin_xp INTEGER NOT NULL DEFAULT 20;
  ALTER TABLE events ADD COLUMN IF NOT EXISTS completion_xp INTEGER NOT NULL DEFAULT 50;

  ALTER TABLE squads ADD COLUMN IF NOT EXISTS campus_id UUID REFERENCES campuses(id) ON DELETE SET NULL;
  ALTER TABLE squads ADD COLUMN IF NOT EXISTS is_active BOOLEAN NOT NULL DEFAULT true;

  -- Backfill default relationships
  UPDATE profiles SET campus_id = (SELECT id FROM campuses WHERE code = 'KIET-I' LIMIT 1) WHERE campus_id IS NULL;
  UPDATE events SET campus_id = (SELECT id FROM campuses WHERE code = 'KIET-I' LIMIT 1) WHERE campus_id IS NULL;
  UPDATE events SET academic_year_id = (SELECT id FROM academic_years WHERE label = '2026' LIMIT 1) WHERE academic_year_id IS NULL;
  UPDATE squads SET campus_id = (SELECT id FROM campuses WHERE code = 'KIET-I' LIMIT 1) WHERE campus_id IS NULL;

  -- Migrate start_time
  UPDATE events SET start_time = event_time WHERE start_time IS NULL AND event_time IS NOT NULL;
  
  -- Generate slugs
  UPDATE events SET slug = lower(regexp_replace(name, '[^a-zA-Z0-9]+', '-', 'g')) || '-' || substr(id::text, 1, 8) WHERE slug IS NULL;

  -- Check for existing PII columns and backfill safely before dropping
...
-- Add missing unique constraints via ALTER TABLE to ensure they exist on pre-existing tables
...
-- (REVOKED public access - this is internal only)
-- ============================================================
...
-- Atomic event registration with capacity lock.
-- ============================================================
...
-- ============================================================
-- DATABASE FUNCTION: fn_student_rank & fn_leaderboard_individual
-- ============================================================
...
BEGIN
  UPDATE xpasses
  SET total_points = (
    SELECT COALESCE(SUM(points), 0) FROM point_transactions WHERE profile_id = NEW.profile_id
  ) WHERE profile_id = NEW.profile_id;

  UPDATE squads
  SET total_points = (
    SELECT COALESCE(SUM(xp.total_points), 0) FROM squad_members sm JOIN xpasses xp ON xp.profile_id = sm.profile_id WHERE sm.squad_id = squads.id
  ) WHERE id IN (SELECT squad_id FROM squad_members WHERE profile_id = NEW.profile_id);
...
BEGIN
  RETURN fn_award_xp(
    p_profile_id, NULL, p_bonus, 'Welcome to XpoX! Registration bonus.',
    'REGISTRATION', p_profile_id, 'registration-' || p_profile_id, NULL
  );
END;
$$;
...
-- ============================================================
-- DATABASE FUNCTION: fn_admin_award_xp
-- ============================================================
...
CREATE POLICY "Users can view own xpass" ON xpasses FOR SELECT USING (
  auth.uid() = profile_id OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('ADMIN', 'SUPER_ADMIN', 'VOLUNTEER', 'EVENT_COORDINATOR'))
);
...
CREATE POLICY "Users can view own event registrations" ON event_registrations FOR SELECT USING (
  auth.uid() = profile_id OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('ADMIN', 'SUPER_ADMIN', 'VOLUNTEER', 'EVENT_COORDINATOR'))
);

ALTER TABLE check_ins ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Check ins viewable by admins and owners" ON check_ins;
CREATE POLICY "Check ins viewable by admins and owners" ON check_ins FOR SELECT USING (
  auth.uid() = profile_id OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('ADMIN', 'SUPER_ADMIN', 'VOLUNTEER', 'EVENT_COORDINATOR'))
);
...
CREATE POLICY "Users can view own points" ON point_transactions FOR SELECT USING (
  auth.uid() = profile_id OR EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('ADMIN', 'SUPER_ADMIN', 'VOLUNTEER', 'EVENT_COORDINATOR'))
);
...
CREATE POLICY "Users can view own redemptions" ON reward_redemptions FOR SELECT USING (auth.uid() = profile_id);
...
DROP POLICY IF EXISTS "Event assignments viewable by everyone" ON event_assignments;
CREATE POLICY "Event assignments viewable by everyone" ON event_assignments FOR SELECT USING (true);
